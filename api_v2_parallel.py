...
class TTSRuntime:
    ...
    def get_pipeline(self) -> TTS:
        if self._pipeline is None:
            with self._lock:
                if self._pipeline is None:
                    self._pipeline = TTS(self.tts_config)
...
class SpeakerRuntime:
    def __init__(
        self,
        speaker_id: str,
        model_runtime: ModelBoundRuntime,
    ):
        self.speaker_id = speaker_id
        self.model_runtime = model_runtime

    def run(self, req: dict):
        runtime = self.model_runtime.get()
...
async def tts_handle(req: dict):
    ...
    try:
        speaker_id = req.get("speaker", "default")
        speaker = speaker_manager.get(speaker_id)
        tts_generator = speaker.run(req)
